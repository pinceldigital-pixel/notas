...
async function ensureSW(){
  if('serviceWorker' in navigator){
    try { return await navigator.serviceWorker.register('./sw-v8.js'); } catch(e){}
  }
  return null;
}
...
function hardInit(){
  ...
  try{
    const fab = document.getElementById('fab');
    const addTop = document.getElementById('addTop');
    if(fab){ fab.onclick = ()=> openModal(); fab.addEventListener('touchend', (e)=>{e.preventDefault(); openModal();}, {passive:false}); }
    if(addTop){ addTop.onclick = ()=> openModal(); addTop.addEventListener('touchend', (e)=>{e.preventDefault(); openModal();}, {passive:false}); }
  }catch(e){}
}
